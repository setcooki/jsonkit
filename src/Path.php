 ...
 * Util std path class
 *
 * perform xpath like string path queries on std objects. see Query for query syntax. instead of using
 * Query for object querying the path syntax allows a more versatile syntax to query objects, manipulate
 * results as part of the path syntax and account for default return values. path queries also allows for using only one
 * method $path->query('path') to query objects instead of Query´s more complex public interfaces
 *
 * path syntax:
 *
 * 1) connectors
 ...
 * see Query for all supported filter operators
 *
 * - selectors:
 ...
 * e.g. the above would return all 2 array elements starting from index 1. see Query for all supported
 * selectors
 *
...
class Path
{
     ...
     * Store or Query
     *
     * @param array|object|Store|Query $object expects object
    ...
    public function __construct($object, $options = null)
    {
        ...
        if($object instanceof Store || $object instanceof Query)
        {
            $this->object = $object;
        }else if(is_object($object)){
            $this->object = new Query($object);
        }else if(is_array($object)){
            $this->object = new Query($object);
        }else{
            throw new \InvalidArgumentException(_("First argument is not a valid object"));
        }
    }
     ...
     *
     * @see Path::__construct
     * @param array|object|Store|Query $object
     * @param null|mixed $options expects optional options
     * @return Path
     * @throws \Exception
     */
    ...
    public function &query($path, $default = null)
    {
        ...
        if($result !== false)
        {
            ...
            if(!is_null($default))
            {
                $default = jsonkit_default($default);
                return $default;
            }else{
    ...
    /**
     * static query function. see Path::query for more
     *
     * @see Path::query
     * @param array|object|Store|Query $object
     * @param string $path expects the query path
    ...
    {
        return static::create($object, $options)->query($path, $default);
    }
    ...
    final protected function execute(&$object, $path)
    {
        ...
        switch($path)
        {
            ...
            :
                if(jsonkit_get_option(self::NORMALIZE_DEFAULT_VALUES, $class))
                {
                    $object = null;
                }
                $object = null;
            ...
            :
                if(($callbacks = jsonkit_get_option(self::ALLOW_CALLBACKS, $class)) !== false)
                {
                    if(is_array($callbacks) && !in_array(trim($m[1]), $callbacks))
                    {
                        throw new InvalidCallbackException(sprintf(_("Callback: %s is not a allowed callback"), $m[1]));
                    }
                    ...
                    if(array_key_exists(4, $m))
                    {
                        ...
                        array_walk($params, function(&$v){ $v = Query::typify(stripslashes($v)); });
                    }else{
                        $params = array();
                    }
                    if(is_callable($callback))
                    {
                        if(is_string($callback))
                        {
                            ...
                            {
                                throw new InvalidCallbackException(sprintf(_("Unable to call callback: %s due to reflection error: %s"), $m[1], $e->getMessage()));
                            }
                        }
                        break;
                    }
    ...
    final protected function &connect(&$object, $connector, $last = false, &$return = false)
    {
        ...
        switch($connector)
        {
            ...
            case '+'
            :
                $object = jsonkit_array_to_object((array)$object + (array)$this->object->get());
                break;
            //merge connector (/store/book/1 & /store/book/2)
            case '&'
            :
                $object = jsonkit_array_to_object(array_merge((array)$object, (array)$this->object->get()));
                break;
