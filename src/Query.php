...
use Setcooki\JsonKit\Exception\InvalidCallbackException;
...
class Query
{
     ...
     * standard empty result exception
     *
     ...
     *
     * @param object|array|string $object expects the object to be searched
    ...
    public function __construct(&$object, $options = null)
    {
        ...
        jsonkit_init_options($options, $class);
        ...
        if(is_array($object) || (is_object($object) && get_class($object) === 'stdClass'))
        {
            if(is_array($object))
            {
                $object = jsonkit_array_to_object($object);
            }
            $this->_object =& $object;
        }else{
            throw new \InvalidArgumentException(_("Value passed as first argument must be of type std object or array"));
        }
    }
    ...
    {
        return new static($object, $options);
    }
    ...
    public static function createFrom($mixed, $options = null)
    {
        if(is_file($mixed))
        {
            return new static(file_get_contents($mixed), $options);
        }else if(is_string($mixed) || is_object($mixed)){
            return new static($mixed, $options);
        }else{
            throw new \InvalidArgumentException(_("First argument is not a valid json string or object"));
        }
    }
    ...
    public static function options($options = null)
    {
        ...
        if($options !== null)
        {
            return jsonkit_set_options($options, $class);
        }else{
            return jsonkit_get_options($class);
        }
    }
    ...
    public function object(&$object = null)
    {
        ...
        if(!jsonkit_get_option(self::RETURN_AS_REFERENCE, $class))
        {
     ...
     * Query::find for syntax and usage. the third argument can be a optional string or array filter values
     * - see Query::filter for more details. this method does not return the query result but reference to
     * $this so multiple queries can be stacked if needed. to get the actual query result use Query::get or
     * any other method that will use the Query::get method to return results. e.g.
     * <code>
     *  $query
     *      ->query('/path')
     *      ->query('/key')
     *      ->get();
     * </code>
     * see Query::find or class description for further details
     *
     * @see Query::find
    ...
    public function query($path, $query = null, $filter = null)
    {
        if($this->_init === true)
        {
            $this->_result =& static::find($this->_object, $path, $query, $filter);
            $this->_init = false;
        }else{
            $this->_result =& static::find($this->_result, $path, $query, $filter);
        }
        return $this;
    }
     ...
     * compatible command like "1,2,3" or "1..5". see class description or Query::filter function for more
    ...
    /**
     * exit query mode, init for new query and return result. preferably use Query::get instead! without
    ...
    public function &init()
    {
        ...
        if(!jsonkit_get_option(self::RETURN_AS_REFERENCE, $class) && is_object($this->_result))
        {
    ...
    public function &first()
    {
        if($this->_result !== null)
        {
            ...
            throw new \BadMethodCallException(sprintf(_("Method: %s used in wrong context"), __FUNCTION__));
        }
    }
    ...
    /**
     * shortcut function and legacy function for Query::get used with second argument index. it is advised
    ...
    public function &at($index)
    {
        if($this->_result !== null)
        {
            return static::filter($this->init(), $index);
        }else{
            throw new \BadMethodCallException(sprintf(_("Method: %s used in wrong context"), __FUNCTION__));
        }
    }
    ...
    /**
     * static find by path interface for Query::query. this method is the preferred way to write complex
     * queries on transient or temporary objects or repeat queries on those without the need to create a class instance
     * or reuse the options set with first class instance since all options are static as well. use Query:.retrieve
     ...
     * see Query::filter for filter syntax
     *
    ...
    public static function &find(&$object, $path, $query = null, $filter = null, &$parent = null)
    {
        ...
        $return     = null;
        $result     = array();
        $default    = jsonkit_get_option(self::DEFAULT_VALUE, $class);
        $operators  = array();

        if($query === null)
        {
            $object =& static::retrieve($object, $path);
            if($filter !== null)
            {
                $object =& static::filter($object, $filter);
            }
            return $object;
        }else{
            if(($object =& static::retrieve($object, $path)) !== false)
            {
                $query = (array)$query;
                ...
                if(sizeof($query) > 1)
                {
                    ...
                    foreach($query as $k => $v)
                    {
                        ...
                        $return =& static::execute($object, $v);
                        foreach($return as &$r)
                        {
                    ...
                    {
                        if(jsonkit_get_option(self::THROW_EXCEPTION, $class))
                        {
                            throw new EmptyResultException(sprintf(_("No result found for query: %s"), $path));
                        }else{
                        ...
                        if($filter !== null)
                        {
                            return static::filter($result, $filter);
                        }else{
                            return $result;
                        }
                    }
                //single query
                    ...
                    static::execute($object, $query[0], $result);
                    if(!empty($result))
                    {
                        if($filter !== null)
                        {
    ...
    /**
     * retrieve value from object passed in first argument at path passed as second argument. see Query::find
    ...
    public static function &retrieve(&$object, $path, $default = false, &$parent = null)
    {
        ...
        if($default === false && jsonkit_get_option(self::DEFAULT_VALUE, $class) !== false)
        {
            $default = jsonkit_get_option(self::DEFAULT_VALUE, $class);
        }
            ...
            if(empty($path))
            {
                ...
                if(stripos($path, '//') !== false)
                {
                    ...
                    foreach($path as $p)
                    {
                        if(substr($p, 0, 2) === '//')
                        {
                            $object =& static::retrieveChilds($object, $p);
                        }else{
                            $object =& static::retrieveObject($object, $p, $parent);
                        }
                    }
                //normal path
                }else{
                    $object =& static::retrieveObject($object, $path, $parent);
                }
            }
        }
        if($object instanceof \Exception)
        {
            ...
            if($default === false && jsonkit_get_option(self::THROW_EXCEPTION, $class))
            {
                throw new EmptyResultException(sprintf(_("No result found for path: %s"), $path));
            }else{
                return jsonkit_default($default);
            }
        }else{
    ...
    /**
     * default internal function to retrieve objects/values at path. see Query::find for path syntax
     *
     * @param object|array $object expects the object to be searched
    ...
    protected static function &retrieveObject(&$object, $path, &$parent = null)
    {
        ...
        foreach(explode('/', trim($path, ' /')) as $p)
        {
            ...
            if($p === '*'){
                $object =& static::reduceChilds($object);
            //get value at path
            }else if($p === '.'){
                // . means current path so do nothing!
            //get first child at path
            }else if($p === '..'){
                $object =& static::firstChild($object);
     ...
     * Query::find for example. when reducing all childs looses reference since childs will be forced into
     * array.
     *
     * @param object|array $object expects the object to be searched
     * @see Query::find
    ...
    protected static function &reduceChilds(&$object)
    {
        ...
        if(is_object($object))
        {
            ...
            if(!empty($tmp))
            {
                $tmp = jsonkit_array_to_object(array_values($tmp));
                return $tmp;
            }else{
                return '__FALSE__';
            }
            ...
            if($tmp !== $object)
            {
                $tmp = jsonkit_array_to_object($tmp);
                return $tmp;
            }else{
                return '__FALSE__';
            }
        ...
        if(is_object($object) || is_array($object))
        {
            foreach($object as $k => &$v)
            {
                if($key === '*')
                {
                    ...
                    {
                        static::retrieveChilds($v, $key, $array, "$path.$k");
                    }
                }
            }
    ...
    /**
     * execute the query part of a query which is the sql like query see in Query::find. this function will
    ...
    final private static function &execute(&$object, $query, &$result = array(), $hash = null)
    {
        if(is_object($object) || is_array($object))
        {
            foreach($object as $k => &$v)
            {
                if(is_object($v))
                {
                    static::execute($v, $query, $result, spl_object_hash($v));
                }else if(is_array($v)){
                    static::execute($v, $query, $result, md5(serialize($v)));
                }else{
                    if(static::assert($k, $v, $query, $object) && !array_key_exists($hash, $result))
                    {
                        $result[$hash] = &$object;
                    }
                }
     ...
     * filter function will throw EmptyResultException or default return value according to class option THROW_EXCEPTION
    ...
    protected static function &filter(&$object, $filter)
    {
        ...
        if($result === '__NULL__')
        {
            ...
            {
                if(($callable = static::isCallable($filter, $object)) !== false)
                {
                    $result = static::invoke($callable[0], $callable[1]);
                }else{
                    if(is_string($filter))
                    {
                        throw new \InvalidArgumentException(sprintf(_("Passed filter: %s is not a recognized filter or callable"), $filter));
                    }else{
                        throw new \InvalidArgumentException(_("Passed filter callable is not a recognized filter or callable"));
                    }
                }
            }
        }
        ...
        if($result !== '__NULL__')
        {
            ...
            {
                if(jsonkit_get_option(self::FILTER_ALWAYS_RETURNS_ARRAY, $class) && is_object($result))
                {
                    $result = array($result);
                }
                return $result;
            }else{
                if(jsonkit_get_option(self::FILTER_ALWAYS_RETURNS_ARRAY, $class))
                {
                    $result = (array)$result;
                }
                return $result;
            ...
            if(jsonkit_get_option(self::FILTER_ALWAYS_RETURNS_ARRAY, $class))
            {
                ...
                if(jsonkit_get_option(self::THROW_EXCEPTION, $class))
                {
                    throw new EmptyResultException(_("No object(s) found for filter"));
                }else{
                    return jsonkit_default($default);
                }
            }
     ...
     * and possible filter operators in Query::find
     *
    ...
    protected static function assert($key, $value, $query, $object = null)
    {
        ...
        if(($m = static::isQuery($query)) !== false && static::isFilter($query) === false)
        {
            //query has operator
            if(sizeof($m) >= 2)
            {
                if(static::isEqual($key, $m[0]))
                {
                    if(array_key_exists(2, $m))
                    {
                        $m[2] = static::typify(trim($m[2]));
                    }else{
                        $m[2] = null;
                    }
                    switch((string)$m[1])
                    {
                        ...
                        case '!%':
                            return (stripos($value, $m[2]) === false) ? true : false;
                        case '>%':
                            return static::isLike($value, trim($m[2], ' %') . '%');
                        case '<%':
                            return static::isLike($value, '%' . trim($m[2], ' %'));
                        case '<>':
            ...
            //query has no operator assuming query for properties (keys) only
                ...
                if(jsonkit_get_option(self::CASE_SENSITIVE_QUERIES, $class))
                {
     ...
     * , see Query::filter for allowed callback arguments, or can be an array with class instances/string name
    ...
    protected static function isCallable($callable, &$object)
    {
        ...
        $params     = array(&$object);
        $callback   = null;
        $callbacks  = jsonkit_get_option(self::ALLOW_CALLBACKS, $class);
        ...
        if(is_string($callable) && (bool)preg_match('=^(([a-z0-9\_]+)(?:(?:\:\:)([a-z0-9\_]+))?)(?:\|(.+)?)?$=is', $callable, $m))
        {
            ...
            {
                $params = str_getcsv(trim($m[4], jsonkit_get_option(self::PARAMETER_ENCLOSURE, $class) . ' '), ',', jsonkit_get_option(self::PARAMETER_ENCLOSURE, $class));
                array_walk($params, function (&$v)
                {
                    $v = Query::typify(stripslashes($v));
                });
        ...
        if(is_callable($callable))
        {
            if($callbacks === false)
            {
                throw new \InvalidArgumentException(sprintf(_("No callbacks allowed as filter argument"), $callback));
            }
            if(is_array($callbacks))
            {
                ...
                if(!is_null($callback))
                {
                    throw new \InvalidArgumentException(sprintf(_("Callback: %s is a valid but not allowed callback"), $callback));
                }
            }
            unset($callback);
            unset($callbacks);
        ...
        if(is_callable($callable))
        {
            if(is_string($callable))
            {
                ...
                try
                {
                    ...
                    if($class->isAbstract())
                    {
                        throw new \InvalidArgumentException(sprintf(_("Unable to invoke class since class: %s is abstract"), $class->name));
                    }
                    $method = $class->getMethod($callable[1]);
                    if(!$method->isPublic())
                    {
                        throw new \InvalidArgumentException(sprintf(_("Unable to invoke class method since method: %s is not public"), $method->name));
                    }
                    if($method->isAbstract())
                    {
                        throw new \InvalidArgumentException(sprintf(_("Unable to invoke class method since method: %s is abstract"), $method->name));
                    }
                    if($method->isStatic())
                    {
                        return call_user_func_array($callable, (array)$params);
                ...
                {
                    throw new InvalidCallbackException(sprintf(_("Unable to call callback: %s due to reflection error: %s"), ((is_object($callable[0])) ? get_class($callable[0]) : $callable[0]) . '::' . $callable[1], $e->getMessage()));
                }
            }
        }else{
            throw new \InvalidArgumentException(_("Passed callable is not a callable"));
        }
    }
