...
use Setcooki\JsonKit\Exception\InvalidActionException;
 ...
 *
 * query the target object added with namespace "ns1" - see Path for full path syntax
 *
 ...
 *
 * please refer to internal actions in the Mapper::action method. if you use custom action registered
 * with the mapper instance you will access the custom action also like "@youraction()". actions can be used together with
 * path query syntax e.g. for conditional queries like, e.g. "${ns1:/path ? @remove(1)}". see Mapper::action
...
class Mapper
{
    ...
    /**
     * register a custom function to be thrown when using internal action "@throw()" and also will override internal
     * EmptyResultException when executing the mapping
     *
    ...
    public function add($mixed, $ns = null)
    {
        if($ns !== null)
        {
            ...
            $ns = jsonkit_get_option(self::DEFAULT_NAMESPACE, $this);
        }

        if($mixed instanceof Store)
        {
            $mixed = $mixed->object();
        }else if($mixed instanceof \ArrayObject){
            $mixed = (array)$mixed;
        }else if(is_string($mixed) && is_file($mixed)){
            if(($mixed = file_get_contents($file = $mixed)) === false)
            {
                throw new InvalidFileException(sprintf(_("Unable to add object from file: %s"), $file));
            }
        }

        if(Json::isJson($mixed))
        {
            $mixed = Json::decode($mixed);
        }
        ...
        if($ns !== '' && $ns !== null)
        {
            if(jsonkit_array_isset($this->_objects, $ns))
            {
                jsonkit_array_set($this->_objects, $ns, jsonkit_array_to_object(array_merge((array)jsonkit_array_get($this->_objects, $ns), (array)$mixed)));
            }else{
                jsonkit_array_set($this->_objects, $ns, jsonkit_array_to_object((array)$mixed));
            }
        }else{
            $this->_objects = jsonkit_array_to_object(array_merge($this->_objects, (array)$mixed));
        }
    ...
    public function registerAction($action, $callback)
    {
        if($this->isCallable($callback))
        {
            ...
            throw new InvalidCallbackException(sprintf(_("Callback for custom action: %s is not a valid callback"), $action));
        }
    ...
    public function functions($dump = false)
    {
        ...
        if((bool)$dump === false)
        {
            return jsonkit_get_option(self::INTERNAL_FUNCTIONS, $class);
        }else{
            if(strtolower(php_sapi_name()) === 'cli')
            {
                echo implode("\n", jsonkit_get_option(self::INTERNAL_FUNCTIONS, $class));
            }else{
                echo '<pre>' . implode("\n", jsonkit_get_option(self::INTERNAL_FUNCTIONS, $class)) . '</pre>';
            }
            return null;
        }
    }
    ...
    public function map($map, $encode = false, $exception = true)
    {
        ...
        try
        {
            if(!empty($map))
            {
                $this->path = new Path(new Store($this->objects()));
                ...
                if(is_string($map))
                {
                    if(is_file($map))
                    {
                        ...
                        {
                            throw new InvalidFileException(sprintf(_("Unable to load mapping object from file: %s"), $file));
                        }
                    }
                    if(Json::isJson($map))
                    {
                ...
                {
                    $this->map = new Store($map);
                }else if($map instanceof Store){
                    $this->map = $map;
                }else if($map instanceof Query){
                    $this->map = new Store($map->object());
                }else if(is_string($map)){
                    return $this->execute($map, '');
                }else{
                    throw new \InvalidArgumentException(_("First argument is not a mappable value"));
                }
                $this->compile($this->map->object());
                if((bool)$encode)
                {
                    return Json::encode($this->map->object());
                }else{
        ...
        {
            if((bool)$exception){
                if($e instanceof EmptyResultException && (jsonkit_is_option(self::CUSTOM_EXCEPTION, $class) && ($_e = jsonkit_get_option(self::CUSTOM_EXCEPTION, $class)) !== null))
                {
                    throw $_e;
                }else{
                    throw $e;
    ...
    {
        return static::create($options)->add($objects)->map($map, $encode, $exception);
    }
    ...
    {
        $class = get_class();
        $regex = jsonkit_regex_delimit(jsonkit_get_option(self::PLACEHOLDER_REGEX, $class));
        ...
        if((!is_object($value) && !is_array($value)) && stripos($value, '@') !== false && !filter_var($value, FILTER_VALIDATE_EMAIL))
        {
            ...
            //action mapping for syntax @map(*)
                ...
                if(in_array($m[1], $this->_internalActions) || array_key_exists($m[1], $this->_customActions))
                {
                    ...
                    throw new InvalidActionException(sprintf(_("Action: %s is not a registered or internal action"), $m[1]));
                }
            }
        }
    ...
    final protected function query(Array $path, $stringify = true)
    {
        ...
        foreach($path as &$p)
        {
            ...
            if((bool)$stringify)
            {
                ...
                {
                    if(jsonkit_get_option(self::FORCE_STRING, $class))
                    {
                        $result[] = "";
                    }else{
                        throw $e;
        ...
        foreach($callbacks as $callback)
        {
            if($this->isRegistered($callback))
            {
                ...
                throw new InvalidCallbackException(vsprintf("Callback: %s is not a registered callback", (array)$callback));
            }
        }
                ...
                if(substr($param, 0, 1) === '$')
                {
                    if(strlen($param) > 1)
                    {
                        if(array_key_exists(((int)substr($param, 1) - 1), $result))
                        {
                            ...
                            throw new \OutOfRangeException(_("Out of range for variable matching for callback"));
                        }
                    }else{
                        $param = $result[0];
                    }
            ...
            :
                if($params !== null)
                {
                    if($this->isRegistered($params[0]) && $this->isCallable($params[0]))
                    {
                        ...
                        throw new InvalidCallbackException(sprintf(_("Action: call and callback: %s is not a registered or valid callback"), $params[0]));
                    }
                }
                break;
            /**
            ...
            :
                if($params !== null)
                {
                    ...
                    catch(EmptyResultException $e)
                    {
                        if(jsonkit_get_option(self::FORCE_STRING, $class))
                        {
                            if(array_key_exists(1, $params)){ $value = Query::typify($params[1]); }
                        }else{
                            throw $e;
                        }
                    }
            ...
            :
                if($params !== null && sizeof($params) >= 2)
                {
                    if($params[0] instanceof \Exception)
                    {
                        ...
                        throw new \InvalidArgumentException(_("Action: throw - first parameter must be instance of Exception"));
                    }
                }else{
                    if(jsonkit_is_option(self::CUSTOM_EXCEPTION, $class) && ($e = jsonkit_get_option(self::CUSTOM_EXCEPTION, $class)) !== null)
                    {
                        throw $e;
                    }
                }
            ...
            :
                if($params !== null && sizeof($params) >= 2)
                {
                    ...
                    if(($value = $this->path->query($this->normalize($path), '__FALSE__')) !== '__FALSE__')
                    {
                          ...
                          $value = Query::typify(((array_key_exists(2, $params)) ? $params[2] : null));
                    }
                }
                break;
            /**
    ...
    protected function invoke($callback, $params = null)
    {
        ...
        if(is_array($callback))
        {
            ...
            {
                throw new InvalidCallbackException(sprintf(_("Unable to invoke callback due to reflection error: %s"), $e->getMessage()));
            }
        }else{
    ...
    protected function isRegistered($callback)
    {
        ...
        if(jsonkit_get_option(self::ALLOW_ONLY_REGISTERED_CALLBACKS, $class))
        {
            if(!is_array($callback))
            {
            ...
            if(isset($callback[1]) && !is_null($callback[1]))
            {
                ...
                if(in_array($callback[0], jsonkit_get_option(self::INTERNAL_FUNCTIONS, $class)) || array_key_exists("func:$callback[0]", $this->_callbacks))
                {
                    return true;
                }
            }
    ...
    public function hasPlaceholder($value)
    {
        ...
        {
            $regex = jsonkit_regex_delimit(jsonkit_get_option(self::PLACEHOLDER_REGEX, $this));
    ...
    protected function stringify($value)
    {
        ...
        {
            return (string)$value;
        }else if(is_bool($value) && jsonkit_get_option(self::STRINGIFY_BOOL_TO_INT, $class)){
            return (string)(int)$value;
        }else if((is_array($value) || is_object($value)) && jsonkit_get_option(self::STRINGIFY_OBJECT_TO_JSON, $class)){
